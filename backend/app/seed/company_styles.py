"""Seed data: Huawei company style + Hardware RF Engineer Intern role.

Based on:
- docs/Huawei Recruitment Process, Corporate Cu v4.pdf
...
HUAWEI_STYLE = {
    ...
    "sample_questions": [
        ...
        # 文化契合 & HR
        "你为什么想加入华为？对华为的业务和文化了解多少？",
        "你对未来 3-5 年的职业规划是什么？如何与华为岗位发展路径结合？",
    ],
    "prompt_context_text": (
        "华为面试评估体系围绕四大核心维度展开：\n"
...
DEFAULT_ROLES = [
    {
        ...
        "business_context": (
            ...
            "华为无线致力于构建 AI 原生的智联基座，依托 5G 及 5G-A 技术，"
...
async def seed_if_empty(session: AsyncSession) -> int:
    """Insert Huawei style if company_style table is empty. Returns count inserted."""
    existing = await session.scalar(select(CompanyStyle).where(CompanyStyle.is_builtin == True))  # noqa: E712
    if existing is not None:
        return 0
    session.add(CompanyStyle(is_builtin=True, **HUAWEI_STYLE))
